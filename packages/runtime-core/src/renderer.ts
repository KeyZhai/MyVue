...
export function createRenderer(options) {
  ...
  function render(initialVnode, container) {
    ...
    // 第一次调用时 initialValue是rootComponent  container是rootContainer
  ...
  function patchKeyedChildren(
    ...
    // 乱序的部分
      ...
      for (let i = s1; i <= e1; i++) {
        ...
        if (newIndex === undefined) {
          ...
          if (newIndex >= maxNewIndexSoFar) {
            //如果当前的newIndex大于maxNewIndexSoFar，说明当前的newIndex是递增的，没有移动
            maxNewIndexSoFar = newIndex;
          } else {
            moved = true;
          }
          // i不能等于0，当i等于0的时候是未建立映射关系，需要创建
          // [0:4,1:5,2:3,3:0]
      ...
      for (let i = toBePatched - 1; i >= 0; i--) {
        ...
        // 说明在preChildren中这个位置的元素不存在，需要创建
    ...
    instance.update = effect(
      () => {
        if (!instance.isMounted) {
          ...
          // render返回的是vnode，this指向p roxy，因为proxy代理了setup返回的数据，所以可以访问到this的数据
          // 获取虚拟节点tree
          ...
          if (next) {
            // 因为next不走mountComponent，所以要将vnode的el赋值给next的el
            next.el = vnode.el;
